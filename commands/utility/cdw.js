...
module.exports = {
    name : 'cdw',
    category : 'Utlity',
    description : 'add member to Clan dyal Walo Clan!',
    run : async(client, message, args) => {
        ...
        if(!message.member.roles.cache.has('931185411399761970')) return message.channel.send({
            embed: {
                color: 16777201,
                description: "Yes You're The Clan leader but you need your clan Role (CDW) to add members!",
        ...
        .setDescription(`<@${target.user.id}> Would you like to join Clan dyal Walo ?\n\n\n`
            + `${yesEmoji} to Accept\n`
            + `${noEmoji} to Refuse`)
        .setTimestamp()
        ...
        client.on('messageReactionAdd', async (reaction, user) => {
            ...
            if (reaction.message.channel.id == channel) {
                if (reaction.emoji.name === yesEmoji) {
                    ...
                    .setColor('RANDOM')
                    .setDescription(`<@${target.user.id}> has accepted the request and joined Clan dyal Walo !`)
                ...
                if (reaction.emoji.name === noEmoji) {
                    ...
                    .setColor('RANDOM')
                    .setDescription(`<@${target.user.id}> has refused the request and didn't join the Clan dyal Walo !`)
                    .setThumbnail('https://cdn.discordapp.com/icons/921346552407142400/a_abb3f32c9e4d68260d962f51175f2db8.gif?size=4096')
                    .setTimestamp()
                    .setFooter(`${message.author.tag}`, message.author.displayAvatarURL({ dynamic: true }))
